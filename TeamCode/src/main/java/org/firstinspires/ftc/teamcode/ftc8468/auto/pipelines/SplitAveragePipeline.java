...
public class SplitAveragePipeline extends OpenCvPipeline {
    ...
    //Telemetry telemetry;
    static ZONE color_zone = ZONE.LEFT; //1;
    int toggleShow = 1;
...
//    Mat original;
    ...
    public SplitAveragePipeline(ZONE_VIEW zoneView)
    {
        ...
        if (zoneView == ZONE_VIEW.LEFT) {
            if ((distance1 > 195) && (distance2 > 190)) {
                ...
                if (max_distance == distance1) {
                    ...
                    color_zone = ZONE.CENTER; // Middle

                } else {
                    //telemetry.addData("Zone 2 Has Element", distance2);
                    color_zone = ZONE.LEFT; // Right
                }
            }
        }
        else {
            if ((distance1 > 195) && (distance2 > 190)) {
                color_zone = ZONE.LEFT; // Left
                max_distance = -1;
            } else {
                max_distance = Math.min(distance1, distance2);
                ...
                if (max_distance == distance1) {
                    //telemetry.addData("Zone 1 Has Element", distance1);
                    color_zone = ZONE.RIGHT; // Middle

                } else {
                    //telemetry.addData("Zone 2 Has Element", distance2);
                    color_zone = ZONE.CENTER; // Right
                }
            }
        }
